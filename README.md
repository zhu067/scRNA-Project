...
pip install -e ".[full]"    # scanpy + liana + decoupler + gseapy + flowsig
```

Windows PowerShell:

```powershell
cd E:\scRNA_Project\scRNA_Project
python -m venv .venv
...
CellChat + event-flow prototype:

```bash
# 1) Export the symbol h5ad and injury_group x cell_type_major pre-check table.
python scripts/run_pipeline.py --config config/default.yaml --local-config config/local.yaml --stages injury_comm

# 2) Run CellChat in R from the repo root.
Rscript scripts/cellchat_GSE146912_PEC.R

# 3) Run FlowSig from the CellChat communication long tables.
python scripts/run_pipeline.py --config config/default.yaml --local-config config/local.yaml --stages flowsig
# or:
python scripts/run_flowsig.py --config config/default.yaml --local-config config/local.yaml

# 4) Build minimal event-flow tables and the network figure from CellChat delta edges.
python scripts/run_pipeline.py --config config/default.yaml --local-config config/local.yaml --stages event_flow
# or:
python scripts/run_event_flow.py --config config/default.yaml --local-config config/local.yaml
```

The event-flow prototype reads `cellchat_R_diff/diff/interaction_weight_edges_nephritis_minus_control.csv`
and writes `tables/PEC_injury_comm/event_flow/{edges,nodes,top_paths}.csv` plus
`figures/PEC_injury_comm/event_flow/network_delta_flow.pdf`.

The FlowSig stage reads the CellChat interaction-level CSV files written under
`tables/PEC_injury_comm/cellchat_R_diff/{control,nephritis}/`,
builds GEMs with NMF, learns the intercellular flow network, and writes
`tables/PEC_injury_comm/flowsig/{flow_var_info,adjacency,network_edges}.csv`
plus `GSE146912_flowsig.h5ad`.
...
| `flowsig.*` | FlowSig 输入 h5ad、CellChat 长表路径、GEM/NMF 与网络学习参数 |
| `qc.*` | 线粒体、过滤、Scrublet |
...
| `flowsig_analysis.py` | 读取 CellChat 长表并运行 FlowSig |
